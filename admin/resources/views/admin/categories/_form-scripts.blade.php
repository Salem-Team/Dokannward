...
(function () {
    ...
    const tileImg = document.getElementById('category-tile-img');
    const tileFallback = document.getElementById('category-tile-fallback');
    const tileWordmark = document.getElementById('category-tile-wordmark');
    const tileName = document.getElementById('category-tile-name');
    const tileSlug = document.getElementById('category-tile-slug');
    const tileLoading = document.getElementById('category-tile-loading');
    const tileMedia = document.getElementById('category-tile-media');
    ...
    function refreshTile() {
        const name = (nameEn?.value || '').trim() || 'New category';
    ...
    function syncFeaturedLabels() {
        const onLabel = document.querySelector('label[for="is_featured"] [data-on]');
        const offLabel = document.querySelector('label[for="is_featured"] [data-off]');
        if (!featuredToggle || !onLabel || !offLabel) return;
        const on = featuredToggle.checked;
        onLabel.hidden = !on;
        offLabel.hidden = on;
    }
    featuredToggle?.addEventListener('change', syncFeaturedLabels);
    syncFeaturedLabels();
})();

/* Homepage logo mark — mirrors brand studio tabs, independent of banner. */
(function () {
    ...
    const urlPreviewWrap = document.getElementById('logo-url-preview');
    const urlPreviewImg = document.getElementById('logo-url-preview-img');
    const tileImg = document.getElementById('category-logo-img');
    const tileFallback = document.getElementById('category-logo-fallback');
    const tileWordmark = document.getElementById('category-logo-wordmark');
    const tileName = document.getElementById('category-logo-name');
    const nameEn = document.getElementById('name_en');
    ...
    function showDropzonePreview(src) {
        if (!src) {
            emptyState?.classList.remove('hidden');
            previewState?.classList.add('hidden');
            previewImg?.removeAttribute('src');
    ...
    function refreshTile() {
        ...
        applyTileSrc(src);
        if (sourceInput.value === 'url') showUrlPreview(src);
    }
